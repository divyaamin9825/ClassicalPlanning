...
class ActionLayer(BaseActionLayer):
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return any(self.parent_layer.is_mutex(p1,p2) for p1 in actionA.preconditions for p2 in actionB.preconditions) and any(self.parent_layer.is_mutex(p2,p1) for p1 in actionB.preconditions for p2 in actionA.preconditions)
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for a in self.parents[literalA]:
            for b in self.parents[literalB]:
                if self.parent_layer.is_mutex(a,b) & self.parent_layer.is_mutex(b,a):
                    continue
                else:
                    return False
        return True
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        for goal in self.goal:
            for cost, level in enumerate(self.literal_layers):
                if goal in level:
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            if AreAllGoalsSeen(last_layer) and NoMutex(last_layer):
                return level
            self._extend()
            level += 1
